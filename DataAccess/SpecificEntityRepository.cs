...
namespace DataAccess
{
    ...
    /// 實體化資料庫物件模型存取類別
    /// </summary>
    public class SpecificEntityRepository<T> where T : class, new()
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        protected readonly DbContext _DbContext;
        protected readonly DbSet<T> _dbset;

        public SpecificEntityRepository(DbContext DbContext)
        {
        ...
        {
            List<T> query = _dbset.Where(WherePredicate).AsEnumerable().ToList();
            try
            {
                if (SortKeys != null && SortKeys.Any())
                {
        ...
        public virtual List<T> Find(List<T> TEntitySet, params Expression<Func<T, IComparable>>[] SortKeys)
        {
            ...
            if (TEntitySet.Count() > 0)
            {
                ...
                try
                {
                    ...
                    query = _dbset.SqlQuery(sSQL, SqlParameters.ToArray()).ToList();
                    if (SortKeys != null && SortKeys.Any())
                    {
                        query.SortBy<T>(SortKeys);
                    }
            ...
            using (var cmd = _DbContext.Database.Connection.CreateCommand())
            {
                try
                {
                    if (CheckQueryStatement(new string[] { Sql }))
                    {
                        ...
                        query = _dbset.SqlQuery(cmd.CommandText, Params).ToList();
                    }
                }
                catch (Exception ex)
                {
