...
namespace WebApiAp
{
    public class WebApiExceptionFilter : ExceptionFilterAttribute
    {
        ...
        public override void OnException(HttpActionExecutedContext context)
        {
            ...
            int errorCode;
            string sErrorMsg = "";
            if (ex is MoneySQMessageWebApiException)
            {
                errorCode = (int)(ex as MoneySQMessageWebApiException).ErrorCode;
                sErrorMsg = ex.GetType().Name + ":" + (ex as MoneySQMessageWebApiException).ErrorMessage;
            }
            else
            {
